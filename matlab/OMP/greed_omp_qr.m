...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Usage
% [s, err_mse, iter_time]=greed_omp_    qr(x,P,m,'option_name','option_value')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input
%   Mandatory:
...
%         nP=norm(P(mask));
%         if abs(1-nP)>1e-3
%             disp('Dictionary appears not to have unit norm columns.')
%         end
        
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                 Random Check to see if dictionary is complex 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

        global conj_atoms;
        complex_dict = false;
%       if norm(imag(P(mask))) > eps && norm(imag(x)) < eps && conj_atoms
        if conj_atoms
            complex_dict = true;
        end
...
while ~done
     ...
     % EDIT: if the dictionary is complex and the signal is real, choose a
     % pair of conjugate atoms
     if complex_dict && I ~= 1 && I ~= m-I+2
         IN = [IN I m-I+2];
         number = 2;
     else
         IN = [IN I];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
     ...
     if strcmp(STOPCRIT,'M')
         ...
         elseif verbose && toc-t>10
            fprintf('Iteration %i. --- %i iterations to go\n',iter ,STOPTOL-iter) 
            t=toc;
         end
    elseif strcmp(STOPCRIT,'mse')
     ...
     elseif strcmp(STOPCRIT,'mse_change') && iter >=2
         if comp_err && iter >=2
             ...
             elseif verbose && toc-t>10
                fprintf('Iteration %i. --- %i mse change\n',iter ,(err_mse(iter-1)-err_mse(iter))/sigsize ) 
                t=toc;
             end
         else
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 s(IN)=R(1:k,1:k)\z(1:k).'; % EDIT: there needs to be non-hermitian transpose
 
 s_temp = s;
 
 % EDIT: ensure that if complex conjugate pairs of atoms were chosen, also
 % the coefficients are complex conjugate
 if complex_dict
    odd = mod(length(s),2);
    a = s(1:floor(length(s)/2)+1);
    if odd
        a_conj = conj(flip(a(2:end)));
    else
        a_conj = conj(flip(a(2:end-1)));
    end
    s = [a; a_conj];
 end
